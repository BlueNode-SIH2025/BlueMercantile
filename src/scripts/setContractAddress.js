/**
 * Simple script to set contract address and network configuration for Web3 features
 * Supports both Ganache local development and testnet deployment
 * Run this in browser console after deploying your contract
 */
...
function setGanacheConfig(contractAddress) {
...
function setSepoliaConfig(contractAddress, rpcUrl) {
  if (!contractAddress || !contractAddress.startsWith('0x') || contractAddress.length !== 42) {
    console.error('❌ Invalid contract address. Please provide a valid Ethereum address starting with 0x');
    return false;
  }
...
window.clearContractAddress = clearContractAddress;
window.setGanacheConfig = setGanacheConfig;
window.setSepoliaConfig = setSepoliaConfig;
